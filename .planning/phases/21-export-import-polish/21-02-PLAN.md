...
must_haves:
  truths:
    - "Slide items in the sequence editor display a 60-80px thumbnail image with caption text beside it"
  ...
  artifacts:
    - path: "src/components/SequenceItemCard.tsx"
      provides: "Enlarged slide thumbnails (60-80px) and batch item icons in sequence editor"
      contains: "w-16 h-16|w-20 h-14|object-cover|loading.*lazy"
...
1. **Enlarge slide thumbnail to 60-80px** (CONTEXT.md specifies "medium preview size 60-80px"):
- Change the slide image container from `w-8 h-8` to `w-16 h-12` (~64x48px, maintains ~4:3 aspect ratio good for slide previews)
- OR use `w-20 h-14` (~80x56px) for a more generous preview -- use your judgment on what looks balanced in the card
...
Run `npx tsc --noEmit` to verify TypeScript compilation. Visually verify that slide items show larger thumbnails and batch items remain visually balanced. Run `npx vitest run` to confirm no test regressions.
  ...
  <done>
Slide items in the sequence editor display 60-80px thumbnails with caption text beside them. Batch items remain visually balanced. All items have consistent card heights. Thumbnails lazy-load with object-cover.
  </done>
</task>
...
3. Visual: sequence editor shows 60-80px slide thumbnails with captions
...
<success_criteria>
- Slide thumbnails in sequence editor are 60-80px with object-cover and lazy loading
