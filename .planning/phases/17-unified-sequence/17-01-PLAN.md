...
must_haves:
  truths:
    ...
    - "Session load shows correct item ordering based on session_items table positions"
  artifacts:
    - path: "src/lib/sequence-api.ts"
      provides: "session_items CRUD, backfill, reorder with position gap closure"
      exports: ["ensureSessionItems", "createBatchSessionItem", "reorderSessionItems", "fetchSessionItems"]
...
Output: `sequence-api.ts` with 4 exported functions, extended session-store with sessionItems state, AdminSession loads and backfills session_items on mount.
</objective>
...
1. **`ensureSessionItems(sessionId: string): Promise<SessionItem[]>`**
   - Fetch all existing session_items for this session, ordered by position ascending.
   - Check if any items with `item_type = 'batch'` exist in the results.
   - If batch-type items already exist: backfill is already done. Return all items sorted by position.
   - If NO batch-type items exist (meaning only slide items or no items at all): perform backfill:
     - Fetch all batches for this session from the `batches` table, ordered by `position` ascending.
     - For each batch, create a session_item with `item_type = 'batch'`, `batch_id` set to the batch id, and `position` set to the batch's index (0, 1, 2, ...).
     - Do NOT modify existing slide session_items. Leave their positions as-is. Slides created in Phase 16 already have correct positions.
     - If there are existing slide items AND new batch items, the final ordering will have batches at positions 0..N-1 (matching their original batch.position order) and slides retaining their current positions. If position collisions occur, that is acceptable -- the reorder function can fix it later, and the UI will sort by position.
     - Insert all new batch items via `.insert()` with `.select()` to get the created rows back.
     - Return ALL session_items (newly created batch items + any pre-existing slide items), sorted by position.
   - IMPORTANT: This function is idempotent. If called multiple times, the second call sees batch-type items exist and returns immediately.

2. **`createBatchSessionItem(sessionId: string, batchId: string): Promise<SessionItem>`**
   - Determine next position: query `max(position)` from session_items for this session, add 1 (or 0 if no items exist).
...
4. **Re-export `fetchSessionItems` from slide-api.ts** (or import and re-export) so consumers can get all items from one module. Alternatively, keep using the existing function from slide-api.ts directly -- choose whichever avoids circular imports.

Import supabase from `../lib/supabase`. Import SessionItem type from `../types/database`.

Do NOT export a `deleteSessionItem` function. Deletion is handled by existing mechanisms: batch deletion cascades to session_items (FK CASCADE), and slide deletion uses `deleteSlide` from slide-api.ts which deletes the session_item row directly. No separate delete function is needed.
...
Verify the file exports the expected functions: `grep "export" src/lib/sequence-api.ts` should show ensureSessionItems, createBatchSessionItem, reorderSessionItems, and fetchSessionItems (re-export).
  </verify>
  <done>
sequence-api.ts exists with ensureSessionItems, createBatchSessionItem, reorderSessionItems exported (plus fetchSessionItems re-exported). No deleteSessionItem export. All functions use supabase client correctly. No type errors.
  </done>
</task>
...
<success_criteria>
- sequence-api.ts exports ensureSessionItems, createBatchSessionItem, reorderSessionItems (plus fetchSessionItems re-export)
