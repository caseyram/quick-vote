...
interface SequenceManagerProps {
  ...
  onAddSlide: () => void;  // Scroll to or open slide upload
}
```
...
8. `handleDeleteItem`:
   - `window.confirm('Delete this item from the sequence?')`.
   - For slides: Call `deleteSlide(item.id, item.slide_image_path!)` from slide-api.ts (handles Storage cleanup + DB delete).
   - For batches: Do NOT delete the batch itself here -- that's a different operation. Only delete the session_item if needed. Actually, per CONTEXT.md: "Items are deletable directly from the sequence list via an inline delete button (with confirmation)". For batch items, deleting from sequence should delete the BATCH (which cascades to session_item). Call the existing batch delete handler passed via props or directly. For slides, delete the slide (Storage + DB).
   - After delete: Call `useSessionStore.getState().removeSessionItem(item.id)`.
   - REVISED APPROACH: Accept `onDeleteBatch` and `onDeleteSlide` callbacks as props instead of handling deletion logic internally. SequenceManager delegates to AdminSession's existing handlers.
...
1. **Import SequenceManager:** Add `import { SequenceManager } from '../components/SequenceManager';`
...
2. **Remove BatchList from draft view:** In the draft view JSX (the `{isDraft && (...)}` block), find the "Questions & Batches" section (around line 1044-1090). Replace the `<BatchList ... />` usage with `<SequenceManager ... />`.
   ...
   <div className="bg-white rounded-lg p-6 space-y-4">
     ...
     <SequenceManager
       ...
       onExpandBatch={(batchId) => {/* navigate to batch detail or expand inline */}}
...
3. **Create handleDeleteSlide handler:** Add a new handler in AdminSession:
   ...
   ```
   Import `deleteSlide` from `../lib/slide-api` and `SessionItem` from `../types/database` (if not already imported).

4. **Keep ImageUploader accessible:** The SlideManager component currently handles image uploading AND slide listing. Since the sequence list now shows slides, we need image upload capability. Two approaches:
   - APPROACH A (simpler): Keep `<SlideManager>` rendered below the sequence for upload-only purposes. It will show its own slide list (redundant but functional). This is acceptable for Phase 17 -- Phase 18+ can refine.
   - APPROACH B (cleaner): Extract just the `<ImageUploader>` from SlideManager and render it inside or below the SequenceManager section. After upload, create the slide and add to sessionItems store.

   Use APPROACH B: Render `<ImageUploader sessionId={session.session_id} onUploaded={handleSlideUploaded} />` inside the sequence section (below the SequenceManager list, above the action buttons or in its own subsection). Create handler:
   ```typescript
   ...
   ```
   Import `createSlide` from `../lib/slide-api`.
...
6. **Keep BatchList for non-draft views:** BatchList is still used in the active view for batch management. Do NOT remove the BatchList import or its usage in non-draft views. Only replace it in the draft view section.
   ...
   IMPORTANT: Actually, review the code -- BatchList is only rendered in the draft view (inside `{isDraft && ...}`). The active view uses separate batch handling. So removing BatchList from draft is safe. Keep the import if it's used elsewhere; remove it if only used in draft.

7. **Handle onExpandBatch:** For now, implement a simple approach -- when a batch card is clicked, navigate the user to expand that batch's details. The simplest approach: set `expandedBatchId` state (reuse existing pattern from BatchList) to show a BatchCard detail panel. However, this introduces complexity. For Phase 17, keep it simple: `onExpandBatch` can be a no-op or show a toast/log. The batch detail expansion (editing questions within a batch) is already handled by the BatchCard component in the active view. In draft mode, clicking a batch in the sequence could eventually open an inline editor. For now, make it a no-op -- the primary interaction is drag-to-reorder and the existing batch editing still works through the active session flow.

   ACTUALLY -- the existing draft view has batch expansion (clicking a batch expands it to show questions, add questions, etc). This is important functionality. We need to preserve batch expansion in the sequence view. Options:
   - Pass expanded state to SequenceItemCard and conditionally render BatchCard content inline
   - Keep a simplified approach where clicking a batch name opens an overlay/modal with batch details

   SIMPLEST APPROACH for Phase 17: When `onExpandBatch` is called, render the existing `<BatchCard>` component below the sequence list (or in a side panel) for the selected batch. This preserves all batch editing functionality without complicating the sortable list.

   Implementation: Add `expandedBatchId` state. When a batch card in the sequence is clicked, set expandedBatchId. Below the SequenceManager, conditionally render a BatchCard for the expanded batch with all its existing props and handlers. This keeps the DnD list clean (no nested DnD contexts) while preserving batch editing.
