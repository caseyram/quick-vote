  ...
  <action>
    **SessionImportExport.tsx:**
    ...
    4. Update `handleExport` to pass templates to `exportSessionData`:
       ...
       Wait — actually `exportSessionData` in Plan 01 receives templates as `Array<{ id: string; name: string; options: string[] }>`. The `ResponseTemplate` type from the store has these fields plus `created_at` and `updated_at`, which is compatible since `exportSessionData` only uses `id`, `name`, and `options`.
  ...
  <action>
    ...
    **In `exportSessionData` describe block:**
    ...
    4. `it('only includes templates referenced by questions')`:
       ...
       - Assert only the `t1` template appears in the exported templates array.
       - Wait — actually `exportSessionData` should include ALL passed templates since the caller is responsible for filtering. Let me re-check Plan 01's design... Plan 01 says exportSessionData receives templates as parameter and maps them. The FILTERING should happen in the caller (SessionImportExport.tsx) or in exportSessionData itself. Per 14-RESEARCH.md Pattern 5, exportSessionData accepts templates as parameter (caller provides). So exportSessionData should filter to only templates referenced by questions in the data. Add this filtering logic check in the test.
