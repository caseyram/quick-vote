...
<task type="auto">
  ...
  <action>
    ...
    2. Extend `QuestionExportSchema` to include `template_id: z.string().nullable()` — this stores the template NAME (not UUID) in the export for human readability and cross-instance portability.
...
<task type="auto">
  ...
  <action>
    ...
    5. In `exportSessionData` function:
       - Add a third parameter: `templates: Array<{ id: string; name: string; options: string[] }> = []` (default empty array for backward compat with existing callers).
       - Shift `sessionName` to fourth parameter: `exportSessionData(questions, batches, templates, sessionName?)`. Wait — this would break existing callers. Instead, use an overload or add templates as the LAST optional parameter: `exportSessionData(questions, batches, sessionName?, templates?)`.
       - Actually, to avoid breaking existing callers, add templates as the last parameter with default: `exportSessionData(questions: Question[], batches: Batch[], sessionName?: string, templates?: Array<{ id: string; name: string; options: string[] }>)`.
    ...
    6. Create `upsertTemplates` async function (private, not exported):
       ...
       - For each template in the array:
         ...
         e. Handle unique constraint race condition (error code '23505'): retry fetch by name, add to map.
