...
must_haves:
  ...
  key_links:
    ...
    - from: "src/components/VoteAgreeDisagree.tsx"
      ...
      via: "vote submission on tap/double-tap"
      pattern: "\\.upsert"
    - from: "src/pages/ParticipantSession.tsx"
...
<objective>
Build the complete participant voting experience: lobby screen, agree/disagree voting, multiple choice voting, double-tap lock-in with haptic + visual confirmation, and the ParticipantSession state machine that drives view transitions. Includes a polling bridge for state updates until Phase 4 adds realtime.
...
<task type="auto">
  ...
  <action>
    ...
    **VoteAgreeDisagree component** (`src/components/VoteAgreeDisagree.tsx`):
    - Props: `{ question: Question; sessionId: string; participantId: string }`.
    ...
    - Single tap flow:
      1. Call `haptic.tap()`.
      2. Submit vote via upsert: `supabase.from('votes').upsert({ question_id: question.id, session_id: sessionId, participant_id: participantId, value: tappedValue, locked_in: false }, { onConflict: 'question_id,participant_id' }).select().single()`.
      3. Update `currentVote` in store with returned data.
      4. Visual highlight on selected button (ring/border + scale).
    - Double tap flow (same option tapped twice):
      1. Call `haptic.confirm()`.
      2. Submit vote with `locked_in: true`.
...
<task type="auto">
  ...
  <action>
    ...
    **Rendering by view state:**
    ...
    - `'voting'`: Check `activeQuestion.type`:
      - `'agree_disagree'`: Render `<VoteAgreeDisagree question={activeQuestion} sessionId={sessionId} participantId={participantId} />`.
      - `'multiple_choice'`: Render `<VoteMultipleChoice question={activeQuestion} sessionId={sessionId} participantId={participantId} />`.
    - `'waiting'`: Show "Waiting for next question..." centered text (the admin hasn't activated a question yet but session is active).
    - `'results'`: Show a simple scrollable list of all questions with their text and type. A full `SessionResults` component will be built in Plan 03; for now render a basic list with question text and a "Session ended" header. This will be replaced by `<SessionResults>` in Plan 03.
  ...
  <done>
    ...
    - Results view shows basic question list when session ended
