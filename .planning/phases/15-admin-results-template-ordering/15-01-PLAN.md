...
Output: AdminSession.tsx and SessionResults.tsx use buildConsistentBarData() with explicit template store lookup — an enhanced version of the SessionReview.tsx pattern that passes live template data for accurate ordering.

Note on 3-arg pattern vs SessionReview's 2-arg pattern: SessionReview.tsx calls buildConsistentBarData(question, aggregated) without a third argument. This works because question.options IS populated from the template during assignment time. However, the admin views intentionally use the 3-arg pattern — buildConsistentBarData(question, aggregated, template?.options) — with a live template lookup from useTemplateStore. This is more robust: it uses the CURRENT template definition rather than the options snapshot stored on the question at assignment time, ensuring admin views always reflect the latest template ordering even if the template was edited after question assignment.
</objective>
...
@.planning/ROADMAP.md
@.planning/STATE.md
@src/pages/SessionReview.tsx (2-arg baseline — admin views enhance this with explicit template lookup)
...
Grep for `useTemplateStore` in SessionResults.tsx — should return matches for both import and usage.
  </verify>
  <done>
